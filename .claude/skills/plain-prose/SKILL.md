...
`web/spreadsheets.diyaccounting.co.uk/public/`, the `PLAN_*.md` design docs, other skill docs,
code comments, and the assistant's own chat responses.
...
- **Active voice, not passive.** "The build generates it," not "it is generated by the build."
  Passive hides who does what and adds words.
...
- **The "not X, it's Y" / "not X but Y" / "not only X but also Y" negation-contrast.** State what
  the thing is, not what it isn't.
...
- **Anthropomorphizing tools and pipelines.** A build does not "want," a deploy does not
...
every claim. Lead with what it does and what the reader gets, in one or two short sentences, and
link out to a `PLAN_*.md` doc for design-level detail instead of reproducing it inline.
...
`README.md` and the public site pages, lead with the benefit and link to a `PLAN_*.md` doc for
the proof instead of reproducing it. Base rules are the Plain English Campaign's
(plainenglish.co.uk). Applies to docs, code comments, site copy, and chat.
